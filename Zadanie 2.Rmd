...
#   arrange(change) %>%
#   slice_head(n=1)
min_change_value <- min(suicides_change$change)

min_country_change <- suicides_change %>%
  filter(change == min_change_value)
...
library(dplyr)
# Dodanie informacji o typie aplikacji
free_apps <- free_apps %>% mutate(Type = "Free", Price = 0)
paid_apps <- paid_apps %>% mutate(Type = "Paid")
norat_apps <- norat_apps %>% mutate(Type = "No_rating")
class(free_apps$Price)

# Połączenie danych
all_apps <- bind_rows(free_apps, paid_apps, norat_apps)

# Zapis połączonego pliku
write.csv(all_apps, "gapps/all_apps.csv", row.names = FALSE)
...
movies <- movies %>%
  left_join(ratings_summary, by = "movieId")
head(movies)

#5.2 Czas dodania ostatniego tagu
last_tag <- tags %>%
  group_by(movieId) %>%
  summarise(
    last_tag_time = max(timestamp),
    .groups = "drop"
  )
last_tag

movies <- movies %>%
  left_join(last_tag, by = "movieId")
head(movies)
...
tags_summary <- tags %>%
  group_by(movieId) %>%
  summarise(
    all_tags = paste(unique(tag), collapse = ", "),
    .groups = "drop"
  )
